...
class BlockGenerator
{
    ...
    public function registerFromMetaBox(MetaBox $meta_box): void
    {
        ...
        $this->blocks[$block_id] = [
            'meta_box'   => $meta_box,
            'fields'     => $fields,
            'config'     => wp_parse_args($block_config, [
                'name'        => $block_id,
    ...
    private function registerBlock(string $block_id, array $block_data): void
    {
        ...
        register_block_type($block_id, [
            ...
            'editor_script_handles' => ['kp-wsf-block-editor'],
        ]);
    }
    ...
    private function renderFieldOutput(array $field, mixed $value): string
    {
        ...
        switch ($type) {
            case 'image':
                ...
                if ($image_url) {
                    $html .= sprintf(
                        '<img src="%s" alt="%s" class="kp-wsf-block-image" />',
                        esc_url($image_url),
                        esc_attr($label)
                    );
                }
            ...
            case 'file':
                ...
                if ($file_url) {
                    $html .= sprintf(
                        '<a href="%s" class="kp-wsf-block-file" target="_blank">%s</a>',
                        esc_url($file_url),
                        esc_html($label ?: basename($file_url))
                    );
                }
                break;
            ...
            case 'gallery':
                ...
                if (!empty($ids)) {
                    ...
                    foreach ($ids as $id) {
                        ...
                        if ($image_url) {
                            $html .= sprintf(
                                '<img src="%s" alt="" class="kp-wsf-block-gallery-image" />',
                                esc_url($image_url)
                            );
                        }
                    }
            ...
            case 'url':
                $html .= sprintf(
                    '<a href="%s" class="kp-wsf-block-link">%s</a>',
                    esc_url($value),
            ...
            case 'email':
                $html .= sprintf(
                    '<a href="mailto:%s" class="kp-wsf-block-email">%s</a>',
                    esc_attr($value),
                    esc_html($value)
                );
                break;
            ...
            case 'checkbox':
                $html .= sprintf(
                    '<span class="kp-wsf-block-checkbox">%s</span>',
                    $value ? esc_html__('Yes', 'kp-wsf') : esc_html__('No', 'kp-wsf')
                );
                break;

            case 'color':
                $html .= sprintf(
                    '<span class="kp-wsf-block-color" style="background-color: %s;"></span>',
                    esc_attr($value)
                );
                break;
                    ...
                    foreach ($value as $row) {
                        ...
                        if (is_array($row)) {
                            foreach ($row as $sub_key => $sub_value) {
                                $html .= sprintf(
                                    '<span class="kp-wsf-block-repeater-item">%s</span>',
            ...
            case 'page_select':
                ...
                if ($post) {
                    $html .= sprintf(
                        '<a href="%s" class="kp-wsf-block-post-link">%s</a>',
            ...
            case 'term_select':
                ...
                if ($term && !is_wp_error($term)) {
                    $html .= sprintf(
                        '<a href="%s" class="kp-wsf-block-term-link">%s</a>',
            ...
            case 'user_select':
                ...
                if ($user) {
                    $html .= sprintf(
                        '<span class="kp-wsf-block-user">%s</span>',
            ...
            default:
                // Text-based fields.
                $html .= sprintf(
                    '<span class="kp-wsf-block-text">%s</span>',
    ...
    {
        $options = [['label' => __('— Select —', 'kp-wsf'), 'value' => '']];
    ...
    private function generateBlockPreview(array $fields): string
    {
        ...
        foreach ($fields as $field) {
            ...
            switch ($type) {
                case 'checkbox':
                    $previews[] = sprintf(
                        'el("p", { key: "%s" }, %s, attributes.%s ? __("Yes", "kp-wsf") : __("No", "kp-wsf"))',
                        $id,
                        $label_json,
                        $id
                    );
                ...
                case 'color':
                    $previews[] = sprintf(
                        'el("p", { key: "%s" }, %s, el("span", { style: { backgroundColor: attributes.%s, display: "inline-block", width: "20px", height: "20px", verticalAlign: "middle" } }))',
                        $id,
                        $label_json,
                        $id
                    );
